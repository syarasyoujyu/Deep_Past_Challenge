...
class EnsembleMBRConfig:
    ...
    base_model_path: str = "google/byt5-small"
    model_a1_path: str = "/kaggle/input/final-byt5/byt5-akkadian-optimized-34x"
    model_a2_path: str = "/kaggle/input/models/mattiaangeli/byt5-akkadian-mbr-v2/pytorch/default/1"
    model_b_path: str = "/kaggle/input/models/mattiaangeli/byt5-akkadian-mbr-v2/pytorch/default/1"
    tokenizer_path: str | None = None
    merge_ratio: float = 0.35
    ties_density: float = 0.50
    ties_epsilon: float = 0.0
    ...
    def __post_init__(self):
        ...
        if self.tokenizer_path is None:
            self.tokenizer_path = self.model_a1_path
        ...
        assert 0.0 <= self.merge_ratio <= 1.0
        assert 0.0 < self.ties_density <= 1.0
...
class TIESMerger:
    def __init__(self, cfg: EnsembleMBRConfig, logger: logging.Logger):
        ...
        self._model_a1_config = None
        self._model_a2_config = None
        self._base_state_dict = None
        self._model_a1_state_dict = None
        self._model_a2_state_dict = None
        self._remove_keys: list[str] | None = None
        self._vector_keys: list[str] | None = None
        self._flat_base = None
        self._flat_model_a1 = None
        self._flat_model_a2 = None
        self._merged_state_dict_cache: dict[float, dict[str, torch.Tensor]] = {}
    ...
    def _load_sharded_state_dict(model_dir: Path) -> dict[str, torch.Tensor]:
        ...
        if index_path is None:
            raise FileNotFoundError(f"No checkpoint index found in {model_dir}")
        ...
        for shard_file in shard_files:
            ...
            if shard_path.suffix == ".safetensors":
                ...
                shard_state = safe_load_file(str(shard_path), device="cpu")
            else:
    ...
    @classmethod
    def _load_state_dict_from_path(cls, model_path: str) -> dict[str, torch.Tensor]:
        ...
        if path.exists():
            ...
            return cls._load_sharded_state_dict(path)
    ...
    @staticmethod
    def _shared_parameter_names(checkpoints: list[dict[str, torch.Tensor]]) -> set[str]:
        if not checkpoints:
            return set()
        shared = set(checkpoints[0].keys())
        for checkpoint in checkpoints[1:]:
            shared &= set(checkpoint.keys())
        return shared

    @staticmethod
    def _shape_compatible_parameter_names(checkpoints: list[dict[str, torch.Tensor]]) -> set[str]:
        shared = TIESMerger._shared_parameter_names(checkpoints)
        compatible = set()
        for key in shared:
            shapes = {tuple(checkpoint[key].shape) for checkpoint in checkpoints}
            if len(shapes) == 1:
                compatible.add(key)
        return compatible

    @staticmethod
    def _shared_weight_key(state_dict: dict[str, torch.Tensor]) -> str | None:
        for key in ("shared.weight", "transformer.shared.weight"):
            if key in state_dict:
                return key
        return None

    @classmethod
    def _remove_keys_for_vector(cls, state_dict: dict[str, torch.Tensor]) -> list[str]:
        shared_key = cls._shared_weight_key(state_dict)
        if shared_key is None:
            return []
        candidate_keys = []
        if shared_key == "shared.weight":
        ...
        if shared_key == "transformer.shared.weight":
            candidate_keys.extend(
                [
        ...
        return [key for key in candidate_keys if key in state_dict]

    @staticmethod
    def _sorted_state_dict_without_keys(
        state_dict: dict[str, torch.Tensor],
        ...
        return OrderedDict(
            sorted(
                ...
                if key not in remove_keys and (include_keys is None or key in include_keys)
            )
        )

    @classmethod
    def _state_dict_to_vector(
        cls,
        state_dict: dict[str, torch.Tensor],
        remove_keys: list[str],
        include_keys: set[str] | None = None,
    ) -> torch.Tensor:
        sorted_state_dict = cls._sorted_state_dict_without_keys(state_dict, remove_keys, include_keys)
        return torch.nn.utils.parameters_to_vector(
            [value.reshape(-1).to(torch.float32) for value in sorted_state_dict.values()]
        )

    @classmethod
    def _vector_to_state_dict(
        cls,
        vector: torch.Tensor,
        ...
        sorted_reference_dict = cls._sorted_state_dict_without_keys(reference_state_dict, remove_keys, include_keys)
        mutable_reference = OrderedDict(
            (key, value.clone())
            for key, value in sorted_reference_dict.items()
        )
        torch.nn.utils.vector_to_parameters(vector, mutable_reference.values())

        restored = {key: value for key, value in mutable_reference.items()}
        shared_key = cls._shared_weight_key(reference_state_dict)
        if shared_key is not None and shared_key in restored:
            for key in remove_keys:
                restored[key] = restored[shared_key]
    ...
    def _topk_values_mask(
        ...
        keep_count = max(1, int(d * density))
        if keep_count >= d:
            return matrix.reshape(original_shape)
        kth_index = d - keep_count + 1
        kth_values, _ = matrix.abs().kthvalue(kth_index, dim=1, keepdim=True)
        mask = matrix.abs() >= kth_values
        masked = matrix * mask
        return masked.reshape(original_shape)

    @staticmethod
    def _resolve_zero_signs(sign_to_mult: torch.Tensor, method: str = "majority") -> torch.Tensor:
    ...
    @classmethod
    def _resolve_sign(cls, matrix: torch.Tensor) -> torch.Tensor:
        sign_to_mult = torch.sign(matrix.sum(dim=0))
        return cls._resolve_zero_signs(sign_to_mult, "majority")

    @staticmethod
    def _disjoint_merge(
        matrix: torch.Tensor,
        sign_to_mult: torch.Tensor | None,
        merge_func: str = "dis-mean",
    ) -> torch.Tensor:
        merge_kind = merge_func.split("-")[-1]
        if sign_to_mult is not None:
            ...
            selected_entries = matrix * rows_to_keep
        else:
            selected_entries = matrix * (matrix != 0)

        if merge_kind == "mean":
            non_zero_counts = (selected_entries != 0).sum(dim=0).float()
            return torch.sum(selected_entries, dim=0) / torch.clamp(non_zero_counts, min=1.0)
        if merge_kind == "sum":
            return torch.sum(selected_entries, dim=0)
        if merge_kind == "max":
            merged = selected_entries.abs().max(dim=0)[0]
            if sign_to_mult is not None:
                merged = merged * sign_to_mult
            return merged
        raise ValueError(f"Merge method {merge_func} is not defined.")

    @classmethod
    def _ties_merge_task_vectors(
        cls,
        ...
        updated = cls._topk_values_mask(flat_task_vectors, density)
        final_signs = cls._resolve_sign(updated)
        merged = cls._disjoint_merge(updated, final_signs, merge_func)
        if epsilon > 0.0:
            merged = torch.where(
                merged.abs() >= epsilon,
                merged,
    ...
    def _ensure_loaded(self) -> None:
        if self._base_config is None:
            self._base_config = AutoConfig.from_pretrained(self.cfg.base_model_path)
        if self._model_a1_config is None:
            self._model_a1_config = AutoConfig.from_pretrained(self.cfg.model_a1_path)
        if self._model_a2_config is None:
            self._model_a2_config = AutoConfig.from_pretrained(self.cfg.model_a2_path)
        if self._base_state_dict is None:
            self.logger.info(f"[TIES] Loading base model weights: {self.cfg.base_model_path}")
            self._base_state_dict = self._load_state_dict_from_path(self.cfg.base_model_path)
        if self._model_a1_state_dict is None:
            self.logger.info(f"[TIES] Loading merge model A1 weights: {self.cfg.model_a1_path}")
            self._model_a1_state_dict = self._load_state_dict_from_path(self.cfg.model_a1_path)
        if self._model_a2_state_dict is None:
            self.logger.info(f"[TIES] Loading merge model A2 weights: {self.cfg.model_a2_path}")
            self._model_a2_state_dict = self._load_state_dict_from_path(self.cfg.model_a2_path)
        shared_keys = self._shared_parameter_names(
            [self._base_state_dict, self._model_a1_state_dict, self._model_a2_state_dict]
        )
        compatible_keys = self._shape_compatible_parameter_names(
            [self._base_state_dict, self._model_a1_state_dict, self._model_a2_state_dict]
        )
        base_keys = set(self._base_state_dict.keys())
        model_a1_keys = set(self._model_a1_state_dict.keys())
        model_a2_keys = set(self._model_a2_state_dict.keys())
        only_base = len(base_keys - shared_keys)
        only_a1 = len(model_a1_keys - shared_keys)
        only_a2 = len(model_a2_keys - shared_keys)
        shape_mismatch = len(shared_keys - compatible_keys)
        if only_base or only_a1 or only_a2:
            self.logger.warning(
                "[TIES] Parameter sets differ across checkpoints; "
                f"shared={len(shared_keys)}, base_only={only_base}, model_a1_only={only_a1}, model_a2_only={only_a2}. "
                "Applying TIES only on shared parameters and inheriting non-shared parameters from the dominant merge model."
            )
        if shape_mismatch:
            self.logger.warning(
                f"[TIES] Shared parameter names with mismatched shapes: {shape_mismatch}. "
                "They will be excluded from vector-space TIES and inherited from the dominant task model."
            )
        if self._remove_keys is None:
            self._remove_keys = self._remove_keys_for_vector(self._base_state_dict)
        if self._vector_keys is None:
            self._vector_keys = sorted(compatible_keys - set(self._remove_keys))
        if self._flat_base is None:
            self._flat_base = self._state_dict_to_vector(
                self._base_state_dict,
                self._remove_keys,
                set(self._vector_keys),
            )
        if self._flat_model_a1 is None:
            self._flat_model_a1 = self._state_dict_to_vector(
                self._model_a1_state_dict,
                self._remove_keys,
                set(self._vector_keys),
            )
        if self._flat_model_a2 is None:
            self._flat_model_a2 = self._state_dict_to_vector(
                self._model_a2_state_dict,
                self._remove_keys,
                set(self._vector_keys),
            )

    def build_model(self, merge_ratio: float, device: torch.device, label: str):
        self._ensure_loaded()
        ratio_a1 = float(merge_ratio)
        ratio_a2 = 1.0 - ratio_a1
        density = float(self.cfg.ties_density)
        epsilon = float(self.cfg.ties_epsilon)
        assert self._base_config is not None
        assert self._model_a1_config is not None
        assert self._model_a2_config is not None
        assert self._base_state_dict is not None
        assert self._model_a1_state_dict is not None
        assert self._model_a2_state_dict is not None
        assert self._remove_keys is not None
        assert self._vector_keys is not None
        assert self._flat_base is not None
        assert self._flat_model_a1 is not None
        assert self._flat_model_a2 is not None

        self.logger.info(
            f"[{label}] Building TIES-merged model "
            f"(ratio_a1={ratio_a1:.3f}, ratio_a2={ratio_a2:.3f}, density={density:.3f}, epsilon={epsilon:.2e})"
        )
        cache_key = round(ratio_a1, 8)
        merged_state_dict = self._merged_state_dict_cache.get(cache_key)
        if merged_state_dict is None:
            dominant_label = "A1" if ratio_a1 >= ratio_a2 else "A2"
            dominant_state_dict = self._model_a1_state_dict if dominant_label == "A1" else self._model_a2_state_dict
            task_vectors = torch.vstack(
                [
                    ratio_a1 * (self._flat_model_a1 - self._flat_base),
                    ratio_a2 * (self._flat_model_a2 - self._flat_base),
                ]
            )
            merged_task_vector = self._ties_merge_task_vectors(
                task_vectors,
                density=density,
                epsilon=epsilon,
                merge_func="dis-mean",
            )
            merged_check = self._flat_base + merged_task_vector
            merged_shared_state = self._vector_to_state_dict(
                merged_check,
                self._base_state_dict,
                self._remove_keys,
                set(self._vector_keys),
            ...
            merged_state_dict = {
                key: value.clone()
                for key, value in dominant_state_dict.items()
            }
            for key, value in self._base_state_dict.items():
                merged_state_dict.setdefault(key, value.clone())
            for key, value in merged_shared_state.items():
                merged_state_dict[key] = value
            self._merged_state_dict_cache[cache_key] = merged_state_dict
            active_ratio = float((merged_task_vector != 0).float().mean().item()) if merged_task_vector.numel() else 0.0
            self.logger.info(
                f"[{label}] TIES merge ready: "
                f"vector_dim={merged_task_vector.numel()}, active_ratio={active_ratio:.4f}, "
                f"removed_keys={len(self._remove_keys)}, dominant_task={dominant_label}"
            )
        else:
            self.logger.info(f"[{label}] Reusing cached merged state dict for ratio={ratio_a1:.3f}")

        dominant_config = self._model_a1_config if ratio_a1 >= ratio_a2 else self._model_a2_config
        model = AutoModelForSeq2SeqLM.from_config(dominant_config)
        missing_keys, unexpected_keys = model.load_state_dict(merged_state_dict, strict=False)
        if hasattr(model, "tie_weights"):
            model.tie_weights()
        if missing_keys:
            self.logger.warning(f"[{label}] Missing keys after merged load: {len(missing_keys)}")
        if unexpected_keys:
            self.logger.warning(f"[{label}] Unexpected keys after merged load: {len(unexpected_keys)}")
...
class ModelWrapper:
    def __init__(
        ...
        if merger is not None and merge_ratio is not None:
            logger.info(
                f"[{label}] Loading merged model on {self.device} "
                f"(ratio={self.merge_ratio:.3f}, tokenizer={tokenizer_path or cfg.tokenizer_path})"
            )
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path or cfg.tokenizer_path)
            self.model = merger.build_model(self.merge_ratio, self.device, label).eval()
        elif model_path is not None:
            logger.info(
                f"[{label}] Loading plain model from {model_path} on {self.device} "
                f"(tokenizer={tokenizer_path or model_path})"
            )
            ...
            raise ValueError("ModelWrapper requires either merger+merge_ratio or model_path")
...
class EnsembleMBREngine:
    def __init__(self, cfg: EnsembleMBRConfig, logger: logging.Logger):
        ...
        self.merger = TIESMerger(cfg, logger)
    ...
    def run(self, test_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("=" * 60)
        logger.info("TIES-Merged + Plain Ensemble × MBR")
        logger.info(f"  Base model        : {cfg.base_model_path}")
        logger.info(f"  Merge model A1    : {cfg.model_a1_path}")
        logger.info(f"  Merge model A2    : {cfg.model_a2_path}")
        logger.info(f"  Plain model B     : {cfg.model_b_path}")
        logger.info(
            f"  Merge ratio       : Model-A={cfg.merge_ratio:.3f} "
            f"(A1={cfg.merge_ratio:.3f}, A2={1.0 - cfg.merge_ratio:.3f})"
        )
        logger.info(
            f"  TIES              : density={cfg.ties_density:.3f}, epsilon={cfg.ties_epsilon:.2e}"
        )
        logger.info(
        ...
        if cfg.dual_gpu:
            ...
            wrapper_b = ModelWrapper(
                cfg,
                logger,
                "Model-B",
                cfg.model_b_device,
                model_path=cfg.model_b_path,
                tokenizer_path=cfg.model_b_path,
            )
...
def print_env(cfg: EnsembleMBRConfig):
    ...
    print(f"Base     : {cfg.base_model_path}")
    print(f"Model A1 : {cfg.model_a1_path}")
    print(f"Model A2 : {cfg.model_a2_path}")
    print(f"Model B  : {cfg.model_b_path}")
    print(
        f"TIES     : ratio={cfg.merge_ratio:.3f} "
        f"density={cfg.ties_density:.3f} epsilon={cfg.ties_epsilon:.2e}"
    )
    print(
...
if __name__ == "__main__":
    ...
    cfg_snap = {
        ...
        "model_a1": cfg.model_a1_path,
        "model_a2": cfg.model_a2_path,
        "model_b": cfg.model_b_path,
        "tokenizer_path": cfg.tokenizer_path,
        "merge_ratio": cfg.merge_ratio,
        "ties_density": cfg.ties_density,
        "ties_epsilon": cfg.ties_epsilon,

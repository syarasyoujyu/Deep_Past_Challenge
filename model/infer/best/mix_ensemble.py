...
class PlainAkkadianDataset(Dataset):
    def __init__(self, frame: pd.DataFrame, logger: logging.Logger):
        self.sample_ids = frame["oare_id"].tolist()
        preprocessor = OptimizedPreprocessor()
        processed = preprocessor.preprocess_batch(frame["transliteration"].tolist())
        self.input_texts = [f"translate Akkadian to English: {text}" for text in processed]
        logger.info(f"Plain prompt dataset: {len(self.sample_ids)} samples")
...
class EnsembleDictEngine:
    ...
    def _build_dataloader(self, dataset, wrapper: ModelWrapper):
    ...
    def _run_one_model(self, wrapper: ModelWrapper, dataset) -> dict[str, list[str]]:
    ...
    def _run_loaded_model(self, wrapper: ModelWrapper, dataset) -> dict[str, list[str]]:
        return self._run_one_model(wrapper, dataset)

    def run(self, dict_frame: pd.DataFrame, plain_frame: pd.DataFrame) -> pd.DataFrame:
        cfg = self.cfg
        cands_per_sample = (
            cfg.num_beam_cands
        ...
        self.logger.info("=" * 60)
        self.logger.info("Mixed Ensemble × MBR  |  Model-A=dict prompt, Model-B=plain prompt")
        ...
        self.logger.info("  Prompt mode       : Model-A=dictionary augmented, Model-B=plain source prefix only")
        self.logger.info(
        ...
        dict_dataset = DictionaryAkkadianDataset(dict_frame, self.logger)
        plain_dataset = PlainAkkadianDataset(plain_frame, self.logger)
        sample_ids = [str(sample_id) for sample_id in dict_dataset.sample_ids]
        if cfg.dual_gpu:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                future_a = executor.submit(self._run_loaded_model, wrapper_a, dict_dataset)
                future_b = executor.submit(self._run_loaded_model, wrapper_b, plain_dataset)
            ...
            pools_a = self._run_model_phase(cfg.model_a_path, "Model-A", cfg.model_a_device, dict_dataset)
            self.logger.info("Phase 2/2 — Model B inference")
            pools_b = self._run_model_phase(cfg.model_b_path, "Model-B", cfg.model_b_device, plain_dataset)
...
def main() -> None:
    ...
    logger.info("Preparing Model-A dictionary-augmented sources")
    tokenizer = AutoTokenizer.from_pretrained(cfg.tokenizer_path)
    dictionary_index = load_dictionary_entries(cfg)
    dict_frame = prepare_dictionary_frame_for_inference(frame, dictionary_index, tokenizer, cfg)
    plain_frame = frame.copy()

    engine = EnsembleDictEngine(cfg, logger)
    results_df = engine.run(dict_frame, plain_frame)
    ...
    cfg_snap = {
        "mode": "model_a_dictionary_model_b_plain",
